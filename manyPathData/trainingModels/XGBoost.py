# -*- coding: utf-8 -*-
# train_xgboost_faulty_sensor_gpsdyn.py — XGBoost with GPS dynamics
# ClearML logs: ONLY Test Accuracy (scalar) + Confusion Matrix (native)
...
def train_xgb_binary_rank(X: pd.DataFrame):
    ...
    test_meta = X.loc[test_mask, ["route_id", "sensor"]].copy()
    test_meta["prob"] = p_test

    # per-route decision: argmax probability
    pred_route = (test_meta
                  .sort_values(["route_id", "prob"], ascending=[True, False])
                  .groupby("route_id")
                  .first()
                  .reset_index()
                  .rename(columns={"sensor": "pred_sensor"}))

    route_true_sub = route_true[route_true["route_id"].isin(pred_route["route_id"])]
    merged = pred_route.merge(route_true_sub, on="route_id", how="inner")
...
def main():
    ...
    df = pd.read_csv(CSV_PATH, usecols=usecols).sort_values(["route_id", "timestamp", "sensor"]).reset_index(drop=True)
    print(f"Rows: {df.shape[0]} | Routes: {df['route_id'].nunique()}")

    X = build_dataset(df)
    cnt = Counter(X["route_id"])
