...
# CSV_PATH = "OnePathData/withFeatures-fixed/easyNoises/full_faulty_dataset_fixed.csv"
...
raw = pd.read_csv(CSV_PATH, usecols=usecols).sort_values(["aircraft", "timestamp", "sensor"]).reset_index(drop=True)
Xdf = build_engineered_dataset(raw)

# -------------------- Route-level labels --------------------
route_true = (Xdf[Xdf["label"] == 1][["aircraft", "sensor"]]
              .drop_duplicates("aircraft")
              .rename(columns={"sensor": "true_sensor"}))
route_true["y"] = route_true["true_sensor"].map(SENSOR2IDX)

routes = route_true["aircraft"].to_numpy()
...
def mask_routes(rts): return Xdf["aircraft"].isin(rts)
...
ignore_cols = {"aircraft", "sensor", "label"}
...
def build_tensor_pack(route_ids):
    ...
    for rid in route_ids:
        ...
        labels.append(int(route_true.loc[route_true["aircraft"] == rid, "y"].iloc[0]))
