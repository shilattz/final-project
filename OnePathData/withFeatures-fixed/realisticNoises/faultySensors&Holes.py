...
# Config
# =========================
INPUT_FILE  = "OnePathData/sensorObservation/sensor_observations2.csv"
OUTPUT_FILE = "OnePathData/withFeatures-fixed/realisticNoises/full_faulty_dataset_realistic.csv"
...
def main():
    ...
    if 'timestamp' not in base.columns and 't' not in base.columns:
        ...
        base = base.sort_values(['aircraft']).reset_index(drop=True)
        base['t'] = base.groupby('aircraft').cumcount()
    ...
    base_run_ids = sorted(base['aircraft'].unique())
    ...
    for new_run_id, (src_run_id, rep_idx) in enumerate(synth_plan):
        ...
        df_run = base[base['aircraft'] == src_run_id].copy().reset_index(drop=True)
        ...
        df_run_noisy['aircraft'] = new_run_id  # reindex synthesized runs
    ...
    rs_dfs = []
    for (rid, sid), g in full_dataset.groupby(['aircraft','sensor'], sort=False):
        gg = g.copy()
    ...
    # =========================
    new_feature_dfs = []
    for rid, group in full_dataset.groupby("aircraft", sort=False):
        group = group.copy()
    ...
    n_runs_out = full_dataset['aircraft'].nunique()
